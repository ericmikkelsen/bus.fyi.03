...
class Contexts{
  ...
  writeJsonFromContext(jsonObj, agency, context, srcFileName){
    //get the source instructions, there should only be 1
    ...
    if ( sourceInstructions ) {
      //get related data
      if ( sourceInstructions.hasOwnProperty("related") ) {
      ...
      if ( sourceInstructions.hasOwnProperty("removeProperties") ) {
        for ( let i = 0; i < sourceInstructions.removeProperties.length; i++ ) {
          let propertyToRemove = sourceInstructions.removeProperties[i];
          delete jsonObj[propertyToRemove];
        }
      }
